...
var API_KEY = "AIzaSyDJ8qtUhVb7se1VMUsNugrAY6EZIEnJoVg";
...
var eg1 =
  "-33.870315,151.196532|-33.869979,151.197355|" +
...
var eg2 =
  "-35.274346,149.130168|-35.278012,149.129583|" +
...
var eg3 =
  "-35.274346,149.094000|-35.278012,149.129583|" +
  "-35.280329,149.129073|-35.280999,149.129293|" +
  "-35.281441,149.129846";

// Example 4 - Drive erratically in Elkin
var eg4 =
  "36.28881,-80.8525|36.287038,-80.85313|36.286161,-80.85369|" +
...
function initialize() {
  ...
  function bendAndSnap() {
    ...
    $.ajax({
      ...
      success: function (data) {
        $("#requestURL").html(
          '<a target="blank" href="' + this.url + '">Request URL</a>'
        );
      ...
      error: function () {
        $("#requestURL").html(
          "<strong>That query didn't work :(</strong>" +
  ...
  function drawDistance() {
    for (var i = 0; i < originals.length - 1; i++) {
      ...
      if (distance > DISTANCE_THRESHOLD_HIGH) {
        ...
        distance < DISTANCE_THRESHOLD_HIGH &&
  ...
  function addPolyWindow(polyline, distance, index) {
    ...
    var content =
      ...
      "<br>" +
      "<strong>Coords:</strong> (" +
      originals[index + 1].lat +
      "," +
      originals[index + 1].lng +
      ")<br><br>" +
    ...
    if (distance > DISTANCE_THRESHOLD_HIGH) {
      content +=
        '<span style="color:#CC0022;font-style:italic">' +
  ...
  function processSnapToRoadResponse(data) {
    ...
    for (var i = 0; i < unsnappedPoints.length; i++) {
      ...
      unsnappedMessage +=
        unsnappedPoints[i].lat + "," + unsnappedPoints[i].lng + "<br>";
    }
  ...
  function drawSnappedPolyline(snappedCoords) {
    ...
    for (var i = 0; i < snappedCoords.length; i++) {
      ...
      var infoWindow = addDetailedInfoWindow(
        marker,
        snappedCoords[i],
        placeIds[i]
      );
